...
def verify_failure(route: str, cassettes: Path, receipt: Path, out: Path) -> None:
    ...
    write_json(out.with_name(out.stem + '-report.json'), {
        ...
        'original_intent_sha256': hashlib.sha256((receipt / 'intent.json').read_bytes()).hexdigest(),
        'original_outbound_sha256': hashlib.sha256((receipt / 'outbound.jsonl').read_bytes()).hexdigest(),
        'original_failure_sha256': hashlib.sha256((receipt / 'failure.json').read_bytes()).hexdigest(),
