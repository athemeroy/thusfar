...
| 数字 | Python `//` 向负无穷取整，`%` 的符号随除数，`int(float)` 向零截断；`round` 在正好一半时取偶数，`round(x, n)` 基于二进制浮点值。 | `general.jsonl` 的 `floor_div/modulo/truncate/round/round_digits` 在两边运行，包括负数和 `2.675`；另有 `round_extremes.jsonl` 的 700 例覆盖 ±1000 位、极大数、最小次正规数、带符号零与溢出。Dart 兼容层显式实现，Python `OverflowError` 映射为 Dart `RangeError`。 | 各业务调用的输入范围和异常传播需随函数 golden 核对。 |
