...
| A0.5 Python 确定性 | 已修复后台发布排序、章界上下文、小传/摘要同章快照、集合散列顺序和浮点累加；本轮另修英文短名索引顺序、去重候选短路判断顺序，五种散列种子回归通过。阿Q及两个部分前缀在并发度 1 下双录一致。暂停阿Q从 4/9 续到 9/9：152/154 产物与 fresh 相同，摘记原字节保留，8 个已缓存模型请求未重复；另外两个产物只在免费 JEV 用量上不同，差异原样记录，A5 的完全相等要求未因此放宽。 | 现有路径可复验；全语料、默认并发度及 A5 用量相等证据不足 |
...
本机附加验证：Python 3.11 独立 `unittest discover` 为 309/309 通过（79.373 秒），两次带追踪录制也各为 309/309；新增续跑收据 8 条回归通过。Jekyll 在排序修复后两遍回放仍匹配 289 个原产物哈希；4 份书籍产物 509 个哈希、6×20 折叠复核通过；全 oracle 凭据扫描为 1,197 个 JSON/JSONL 文件，未发现密钥。Dart 分析零提示，测试 73 通过、549 跳过、0 失败。解析语料、special、JavaScript 和翻译目录的先前验证仍有效。
...
先在现有共享账本内补可负担的真实 HTTP 成功录制：只读计算显示连接测试单次预留约 ¥0.00544，批注与问书的小上下文也可能容纳，正在准备隔离录制工具，尚未发送这些真实请求。之后重新冻结全部录制输入并复验。五书完整录制仍有预算缺口；Jekyll 末段、法文其余段、日文和中文长篇、默认并发度及历史人工多摘记覆盖仍未完成。A0 门槛未过之前不开始业务 Dart 移植。
