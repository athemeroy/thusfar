...
- 修法：兼容层将 IEEE-754 浮点数拆成精确二进制分数计算取整和混合比较；大结果返回 `BigInt`，`PyJson` 写出完整十进制；负数整除和取模接受 `int`/`BigInt` 并用 `BigInt` 算后收窄。62 个新增 Python/Dart 成对边界样本覆盖大整数连算、NaN、符号零、布尔键排序及异常映射。后续业务移植要证明原生 `int` 的输入范围。
