...
- A separate full-suite run observed one complete-book hash mismatch in the thread-worker HTTP receipt. Successful retries did not close the issue. Retained stress capture 22 in `/tmp/thusfar-fresh-stress-20260926/22/artifacts` and an independent full-suite diagnostic both reproduced the exact mismatch: `work/usage.json` contained `jev_calls: 51`, while final `status.json` and the reference contained 52. Every other field and all 153 other artifacts matched. The persistence fix is documented below; all affected reference captures must be regenerated before acceptance.

## 2026-09-26: Persist settled usage independently of job completion order

- Symptom: a biography guard can finish after the last model response writes `work/usage.json`. The later final status sees the new guard count, but the restorable usage file retains the older count. The same tape therefore occasionally produced different final bytes even though final in-memory counts and all business artifacts were equal.
- Cause: only `Runner.count()` persisted usage; background biography verification increments `jev_calls` after its model response, and `Runner.status()` previously wrote only the status file. The separate stale `jev` telemetry snapshot had the same persistence gap.
- Fix: `Runner.status()` acquires `self.lock` before `status_lock`, captures usage once, and writes the same deep-copied snapshot to both files. This covers settled complete and partial-run progress boundaries without changing requests, token accounting, or normalization. The direct cancellation/error handlers in `run_book` remain separate paths; this change does not establish their accounting equality.
- Validation: two event-controlled Python 3.11 regressions fail on the previous source and pass after the fix. They run the real biography job with both guard/model completion orders, compare both persisted files, and reload a partial run to prove paid counters are not recounted. An independent read-only review found no reverse lock acquisition. Previous ordinary-function capture candidates are rejected because one full test run failed, even though the two function output trees matched. Book, paused-snapshot, resume, worker, and ordinary-function references will be regenerated against this fix.
